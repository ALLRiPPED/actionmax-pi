...
// joystick
static Sint16 xpos = 0, ypos = 0, jrelx = 0, jrely = 0, xmov = 0, ymov = 0;
static Sint8 js_sen = 5;
...
bool singe::init()
{
	...
	if (bSuccess)
	{
		...
		g_SingeIn.draw_border = draw_border;
...
void singe::input_enable(Uint8 input)
{
	...
	if (g_pSingeOut) // by RDG2010
		g_pSingeOut->sep_call_lua("onInputPressed", "ii", input);
}
...
void singe::input_disable(Uint8 input)
{
	...
	if (g_pSingeOut) // by RDG2010
		g_pSingeOut->sep_call_lua("onInputReleased", "ii", input);
}
...
bool singe::handle_cmdline_arg(const char *s)
{
	...
	if (strcasecmp(s, "-js_range") == 0) {
		get_next_word(t, sizeof(t));
		i = atoi(t);
		...
		{	// traverse the keydef array for mapped keys.
			...
			if (key == keydefs[SWITCH_PAUSE][0] || key == keydefs[SWITCH_PAUSE][1]) 
			{
				if (g_game->get_pause_key_flag())  // rdg
				{
					...
					//input_disable(SWITCH_PAUSE);  // -1 is for no mouse.
